...
> **This is a dated snapshot, not a living document.** Several gaps described below have
> since been closed — OFREP, the TypeScript SDK, the evaluation spec + conformance
> vectors, approvals/change requests, and scoped RBAC all landed after the research date.
> [REMAINING-WORK.md](REMAINING-WORK.md) is the living backlog derived from this research;
> trust it over this file where they disagree. Overtaken claims below are annotated
> `[OVERTAKEN]` rather than rewritten, so the research record stays intact.
> See also [design-review-2026-08-24.md](design-review-2026-08-24.md).

**Research date:** 2026-08-22
**Baseline:** Switchboard as built (Spring Boot / WebFlux / R2DBC / Postgres backend, React dashboard, Expo companion app), verified against the source tree rather than the README. *[OVERTAKEN — the Expo companion was deleted on 2026-08-24; the dashboard is the only management surface. See [DECISIONS.md](DECISIONS.md#product-scope).]*
...
**Streaming for everyone.** SSE change delivery with `Last-Event-ID` catch-up (*nuance:
catch-up is a fresh full `put` on reconnect, not a replay of missed patches*), fanned out across instances via Postgres `NOTIFY`, at no tier. Compare: **Flagsmith's real-time SSE is Enterprise-only** and its payload is just a timestamp ping (the SDK must re-fetch); **Unleash streaming is beta and Enterprise Edge only**, and OSS Unleash Edge is **EOL 2026-12-31**; **ConfigCat's cloud cannot push at all** — "realtime" means webhook to your endpoint, then you call `forceRefresh()`, with true SSE available only from the self-hosted Proxy. Switchboard's `patch`-per-change is also better than Flagsmith's re-fetch ping. Getting this from one Postgres instead of a Redis/Kafka tier is a real operational advantage for a self-hoster.
...
**Architectural note on effort.** Typed attributes are an **M**, not an **S**, because the change reaches further than it looks: `EvalContext.attributes`, `Clause.values`, the JSONB round-trip for `TargetingConfig` (see the known hazard around records with derived JSON properties), the OpenAPI contract, the dashboard rule builder, the AI tool schema in `FlagChangeToolSchema`, and — most importantly — the cross-language evaluation contract. ~~**`spec/conformance/` is currently an empty directory**~~ *[OVERTAKEN — the spec and 201
conformance vectors landed 2026-08-22, as the roadmap table below records.]* Write the spec *before* the operator work, not after; the operator set is the thing the spec is mostly about, and every SDK will encode it.
...
| Retention controls | ~ → **configurable** *[OVERTAKEN 2026-08-25 — `switchboard.events.retention-months`, default 3, clamped at 1; whole partitions are dropped, so lowering it is immediate and destructive]* | LaunchDarkly (30-day contexts, 30-day exposures, tiered AI-data retention, 60-day playground data), ConfigCat (7–750 days audit by plan), PostHog (7 days → 60 months) | Compliance floor and cost ceiling | **S** |
